...
@receiver(m2m_changed, sender=Item.conversions.through)
def on_item_conversions_change(sender, instance, action, **kwargs):
...
def on_item_tags_change(sender, instance, action, **kwargs):
    if action in ["post_add", "post_remove", "post_clear"] and instance.global_item_id:
